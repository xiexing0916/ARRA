...
class ChameleonXLLMXForConditionalGeneration(ChameleonForConditionalGeneration):
    ...
    def forward(self, input_ids=None, labels=None, training=True, **kwargs):
        ...
        if self.config.z_loss_weight > 0:
            ...
            # next_token_greedy = greedy_decode(probs)
            # new_token = torch.tensor([[next_token_greedy]], device="cuda")
            # print(input_ids.shape)
            # print(new_token.shape)
            # new_token_sequence = torch.cat((input_ids, new_token), dim=1).cpu().numpy().flatten().tolist()
