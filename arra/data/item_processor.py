...
def center_resize(pil_image, crop_size):
    width, height = pil_image.size

    # 计算填充后的尺寸
    if width < height:
        new_size = (height, height)  # 将宽度填充至与高度一致
        delta_w = (height - width) // 2
        padded_image = Image.new("RGB", new_size, (0, 0, 0))  # 创建新图像
        padded_image.paste(pil_image, (delta_w, 0))  # 填充
        pil_image = padded_image
    else:
        new_size = (width, width)  # 将高度填充至与宽度一致
        delta_h = (width - height) // 2
        padded_image = Image.new("RGB", new_size, (0, 0, 0))  # 创建新图像
        padded_image.paste(pil_image, (0, delta_h))  # 填充
        pil_image = padded_image
...
def var_center_crop(pil_image, crop_size_list, random_top_k=1):
    ...
    # )[1]
    crop_size = (512, 512)
    return center_resize(pil_image, crop_size)
...
class FlexARItemProcessor(MMConvItemProcessor):
    ...
    def __init__(
        self,
        tokenizer="Alpha-VLLM/Lumina-mGPT-7B-512",
