...
class FinetuneSolverBase(ABC):
    ...
    def get_args_parser(cls):
        ...
        parser.add_argument("--visual_feature_base_dir", default="./pre_tokenization/mimic_impression", help="path for visual feature (CLIP, SAM)")
        parser.add_argument(
            "--cache_ann_on_disk",
            action="store_true",
        ...
        parser.add_argument(
            "--ckpt_max_keep", default=15, type=int, help="maximum number of checkpoints to keep, <=0 means keep all"
        )
    ...
    def build_model(self) -> (nn.Module, Tokenizer):
        ...
        if hasattr(unwrapped_model, "get_trainable_params"):
            ...
            #
            # lora_dropout = 0.05
            # lora_target_modules_list = "q_proj,k_proj,v_proj,o_proj" #"self_attn", "ffn", q_proj,v_proj
            # lora_target_modules = find_linear_layers(
            #     unwrapped_model, lora_target_modules_list.split(",")
            # )
            ...
            # )
            # unwrapped_model = get_peft_model(unwrapped_model, lora_config)
            #
            # for n, p in unwrapped_model.named_parameters():
            #     p.data = p.data.to(self.mixed_precision_dtype)
            #     # promote_param_to_fp32(p)
            #     # if p.requires_grad:  # ���Ä�p
            #     #     print(f"Parameter: {n}, Precision: {p.dtype}")
            #     # else:
            #     #     print(f"without train Parameter: {n}, Precision: {p.dtype}")
            #     # promote_param_to_fp32(p)
            #     if any(
            #             [
            #                 x in n
            #                 for x in ["norm", "lm_head", "embed_tokens"]
            #             ]
            #     ):
        ...
        # ----------------
        self.is_peft = getattr(unwrapped_model, "is_peft", True)  # todo
    ...
    def setup_fsdp_sync(self, model: nn.Module, data_parallel: str, precision: str,
                        grad_precision: Optional[str]) -> FSDP:
    ...
    def train_one_epoch(
        ...
        for data_iter_step, batch_data in enumerate(
            ...
            examples, labels, image_path = batch_data
            ...
            with {
                ...
                c_loss, additional_loss_dict = self.model(examples, labels, image_path, self.args.visual_feature_base_dir)
                # c_loss2, additional_loss_dict2, _ = self.model(new_token_sequence, labels)
            loss = c_loss
            # loss = c_loss + c_loss2
            ...
            # c_loss_value2 = c_loss2.item()
            # c_loss_value_all = c_loss.item() + c_loss2.item()
